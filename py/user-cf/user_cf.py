...
# 1. 获得用户和用户之间的相似度
# 1.1 正常逻辑的用户相似度计算
#d {"user":{"item1":"rating","item2":"rating"},....}
    ...
    for i, users in item_users.items():
        for u in users:
            ...
            for v in users:
                ...
                #C {"user1":{"user2":float,"user3":float}}
    del item_users
    # 计算最终的相似度
...
if __name__ == '__main__':
    ...
    # print(rank)
    need=(sorted(rank.items(), key=lambda x: x[1], reverse=True)[0:10])
    print(dict(need))
